...
export default function Moodboard({ projectId, project, assets = [], addAsset, removeAsset, updateTitle, user, setView, deleteProject }: any) {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-[#121212] text-white font-sans flex flex-col" {...getRootProps()}>
      <input {...getInputProps()} />

      {/* Red Background Gradient coming from bottom to top */}
      <div className="absolute inset-0 pointer-events-none -z-10" style={{ background: 'linear-gradient(to top, #ff2300 0%, #3a0900 15%, #121212 40%, #121212 100%)' }} />

      {/* Massive Background Typography */}
      <div className="absolute top-0 left-0 w-full px-16 pt-8 pointer-events-none z-0 user-select-none">
        <textarea 
          className="font-bold text-[10vw] leading-[0.8] text-[#1a1a1a] tracking-tighter uppercase disabled:bg-transparent placeholder-[#1a1a1a]/50 w-full outline-none bg-transparent pointer-events-auto resize-none h-[40vh] overflow-hidden"
          value={project?.name || ''}
      ...
      {/* Tools & Search row above the images */}
      <div className="absolute top-[35vh] right-16 z-20 flex items-center justify-end gap-6 w-full px-16 pointer-events-none">
        <div className="flex items-center gap-4 text-[#555] pointer-events-auto">
           <LayoutGrid onClick={() => setLayout('grid')} className={`w-6 h-6 cursor-pointer transition-colors ${layout === 'grid' ? 'text-[#FF4500]' : 'hover:text-white'}`} />
           <LayoutPanelLeft onClick={() => setLayout('masonry')} className={`w-6 h-6 cursor-pointer transition-colors ${layout === 'masonry' ? 'text-[#FF4500]' : 'hover:text-white'}`} />
           <Filter 
             onClick={() => setFilterType(prev => prev === 'all' ? 'image' : prev === 'image' ? 'video' : 'all')} 
             className={`w-6 h-6 cursor-pointer transition-colors ${filterType !== 'all' ? 'text-[#FF4500]' : 'hover:text-white'}`} 
           />
           {filterType !== 'all' && <span className="text-[10px] uppercase font-bold text-[#FF4500] ml-[-8px]">{filterType}s</span>}
        </div>
        <div className="flex items-center bg-transparent border border-[#555] rounded-full px-4 py-2 w-64 pointer-events-auto">
          <input 
            type="text" 
            placeholder="SEARCH"
          ...
          />
          <Search className="w-4 h-4 ml-2 text-[#555]" />
        </div>
        
        {/* Whiteboard Toggle & Delete Project */}
        <div className="flex items-center gap-2 pointer-events-auto">
          <button onClick={(e) => { e.stopPropagation(); setView('whiteboard'); }} className="border border-white/20 text-white/40 hover:text-white hover:border-white/50 h-8 px-4 rounded-full flex items-center text-[10px] uppercase tracking-widest font-bold transition-all">
            <PenTool className="w-3 h-3 mr-2" /> Draw
          </button>
          <button 
            onClick={(e) => { e.stopPropagation(); deleteProject && deleteProject(); }} 
            className="border border-red-500/30 text-red-500/70 hover:text-red-500 hover:border-red-500 hover:bg-red-500/10 h-8 w-8 rounded-full flex items-center justify-center transition-all"
      ...
      {isDragActive && (
        <div className="absolute inset-0 z-50 bg-[#ff2300]/80 backdrop-blur-sm border-4 border-dashed border-white/40 flex flex-col items-center justify-center pointer-events-none">
           <span className="font-bold text-4xl text-white tracking-widest uppercase">Drop Images Here</span>
        </div>
      )}

      {/* Center Images Area */}
      <div className="flex-1 mt-[45vh] w-full px-16 overflow-y-auto no-scrollbar pb-32 z-10 relative pointer-events-auto">
             ...
             {photos.map((p: any) => (
               <ContextMenu key={p.key}>
                 <ContextMenuTrigger asChild>
                   ...
                   >
                     {p.type === 'video' ? (
                       <motion.video layoutId={`media-${generalId}-${p.key}`} src={p.src || undefined} autoPlay loop muted playsInline className="w-full h-auto object-cover transition-transform hover:scale-105 duration-700" />
                     ) : (
             ...
             {photos.map((p: any) => (
                <ContextMenu key={p.key}>
                  <ContextMenuTrigger asChild>
                    ...
                    >
                       {p.type === 'video' ? (
                         <motion.video layoutId={`media-${generalId}-${p.key}`} src={p.src || undefined} autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover transition-transform hover:scale-105 duration-700" />
                       ) : (
      ...
      {/* Bottom Left Avatar */}
      <div className="absolute bottom-10 left-16 z-20 flex items-center cursor-pointer group" onClick={() => navigate('/')}>
        {user?.photoURL ? (
      ...
      {/* Bottom Right Logo */}
      <div className="absolute bottom-10 right-12 z-20 opacity-60 pointer-events-none flex flex-col items-center">
